...
//  Найти всех пользователей младше 30 лет с именем, начинающимся на букву "A"
    public static void main(String[] args) {
        ...
        List<User> userList30 = findUserByPredicate(userList, user -> user.getAge() < 30 && user.getName().charAt(0) == 'A'); // .toLowerCase(). ignoreCase()
    ...
    private static List<User> findUserByPredicate(List<User> userList, Predicate<User> predicate) {
        ...
        for (User user : userList) {
            if(predicate.test(user)){ // лож или истину если пользователь проходит тест занчит true то иф срабатывает и мы добавляем пользователя в лист
                resList.add(user);
            }
        }
        return resList;
